pipeline {
  ...
  environment {
    ...
    SERVICE_HOST = "${JUMP_HOST}"  // 通过 ubuntu-master Tailscale IP 访问 k3s NodePort
    GITEA_URL  = "http://admin:***@100.76.15.64:13000/admin/supply-chain-platform.git"
    GIT_BRANCH = "main"
  }
  stages {
    ...
    stage('Verify endpoints') {
      steps {
        sh '''
          set -euo pipefail
          for u in http://${SERVICE_HOST}:${NODE_PORT}/ http://${SERVICE_HOST}:${NODE_PORT}/api/health http://${SERVICE_HOST}:${NODE_PORT}/api/shops "http://${SERVICE_HOST}:${NODE_PORT}/api/shops?platform=huaxiang"; do
            code=$(curl -s -o /dev/null -w '%{http_code}' --max-time 30 "$u" || echo ERR)
            echo "$code $u"
            [ "$code" = "200" ] || exit 20
          done
        '''
